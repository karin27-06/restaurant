...
class UpdateSupplierRequest extends FormRequest{
    ...
    public function rules(): array{
        return [
            'name' => 'required|string|max:150|unique:suppliers,name,' . $this->route('supplier')->id,
            'ruc' => ['required', 'digits:11', 'unique:suppliers,ruc,' . $this->route('supplier')->id],
            'address' => 'required|string|max:255',
            'phone' => ['required', 'regex:/^[0-9]{7,11}$/'],
