...
const handleFileChange = async (event: Event) => {
    ...
    try {
        ...
        await axios.post('/api/categorias/import-excel', formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        })

        toast.add({ severity: 'success', summary: 'Importación exitosa', detail: 'Categorías importadas correctamente.' })
        emit('import-success')
        target.value = ''
    } catch (error) {
