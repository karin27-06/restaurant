<template>
    <div class="flex flex-wrap items-center gap-3 p-2">
        <!-- Exportar Excel -->
        <a href="/panel/reports/export-excel-dishes" download>
        ...
        <!-- Exportar PDF -->
        <a href="/panel/reports/export-pdf-dishes" download>
...
const handleFileChange = async (event: Event) => {
    ...
    try {
        ...
        await axios.post('/api/platos/import-excel', formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        })

        toast.add({ severity: 'success', summary: 'Importación exitosa', detail: 'Platos importados correctamente.' })
        emit('import-success')
        target.value = ''
    } catch (error) {
