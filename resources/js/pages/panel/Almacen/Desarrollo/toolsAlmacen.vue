<template>
    <div class="flex flex-wrap items-center gap-3 p-2">
        <!-- Exportar Excel -->
        <a href="/panel/reports/export-excel-almacenes" download>
        ...
        <!-- Exportar PDF -->
        <a href="/panel/reports/export-pdf-almacenes" download>
...
const handleFileChange = async (event: Event) => {
    ...
    try {
        ...
        await axios.post('/api/almacenes/import-excel', formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        })

        toast.add({ severity: 'success', summary: 'Importación exitosa', detail: 'Almacenes importadas correctamente.' })
        emit('import-success')
        target.value = ''
    } catch (error) {
